...
class ArticleColumn(models.Model):
    ...
    文章栏目的 Model
    """
    title = models.CharField(max_length=100, verbose_name='栏目标题',blank=True)        # 栏目标题
...
class ArticlePost(models.Model):
    ...
    column = models.ForeignKey(ArticleColumn,null=True,blank=True,on_delete=models.CASCADE,related_name='article')  # 文章栏目的 “一对多” 外键
    tags = TaggableManager(blank=True,verbose_name='文章标签')      # 文章标签    # 采用 Django-taggit 库
    # models.CharField 为字符串字段，用于保存较短的字符串，比如标题
    # CharField 有一个必填参数 max_length，它规定字符的最大长度
    title = models.CharField(max_length=100,verbose_name='文章标题')
    # body = models.TextField(verbose_name='文章内容')        # 保存大量文本使用 TextField
    body = RichTextUploadingField() #RichTextField()
    total_views = models.PositiveIntegerField(default=0,verbose_name='文章浏览量')    # 浏览量
    likes = models.PositiveIntegerField(default=0,verbose_name='文章点赞数')    # 文章点赞数
